...
# Load real data + full sample
df = pd.read_csv("chatgpt_daily_tweets.csv")
sample = df['text'].sample(n=150, random_state=42)
...
filename = f"risk_report_150tweets_{timestamp}.csv"
pd.DataFrame([{
    "batch_size": 150,
    "sample_seed": 42,
    "timestamp": timestamp,
    "full_audited_report": result
}]).to_csv(filename, index=False)

print(f"\n=== FULL BATCH COMPLETE ===\nSaved to: {filename}")
print("Sample of result (first 500 chars):")
